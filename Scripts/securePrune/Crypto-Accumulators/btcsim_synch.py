...
class Miner:
	def __init__(self, miner_id, hashrate, verifyrate, blocksize, seed_block, event_q,A0,n,S,A,proofs,t):
		...
		self.k = 500
		self.k_c = 275
		self.num_affirmatins = 0
		self.pulse_block_length = 1000
		...
		self.num_blocks_securePrune = dict()
		self.num_blocks_coinPrune = dict()
		self.num_blocks_bitcoin = dict()
	...
	def mine_block(self):
		...
		self.S_d[self.blocks[self.chain_head].height+1] = n_inputs
		self.S_a[self.blocks[self.chain_head].height+1] = n_outputs
		self.state[self.blocks[self.chain_head].height+1] =  self.state[self.blocks[self.chain_head].height] - n_tx * n_inputs + n_tx * n_outputs
		
		#t_size = n_tx * (n_inputs + n_outputs) * 150
		if self.miner_id == 0:
			...
			self.num_blocks_securePrune[self.blocks[self.chain_head].height], self.num_blocks_coinPrune[self.blocks[self.chain_head].height], self.num_blocks_bitcoin[self.blocks[self.chain_head].height] = self.calculate_num_blocks()
			if self.blocks[self.chain_head].miner_id in [2,3,4,5,6,9,11]:
		...
		while rerun == 1:
			...
			for t_block in self.blocks_new:
				...
				if validity == 1: 
					...
					#rerun = 1
				if validity == 0 and self.miner_id != 1000:
	...
	def receive_event(self, t, t_event):
		...
		if t_event.action == 'getstate':
			#print(t_event.orig,t_event.payload)
			self.send_state(t_event.orig, t_event.payload)
			
		if t_event.action == 'state':
			self.state = t_event.payload
			#print(self.state, len(self.state),t_event.orig)
			self.request_tailchain(t_event.orig, t_event.payload)
			
		if t_event.action == 'tailchain':
			#print(t_event.orig, t_event.dest)
			self.send_tailchain(t_event.orig,t_event.payload)

	
	def send_event(self, t, to, action, payload):
		t_event = Event(to, self.miner_id, action, payload)
		heappush(self.event_q, (t, t_event))
		if t_event.dest == 1000 and t_event.action == 'addblock' and t_event.payload.height <=50:
	...
	def request_state(self, to, state_id):
		for t_link in self.links:
			if t_link.dest == to:
				t_arrival = t_link.occupy(self.t,0)
				#print(state_id,t_link.dest)
				self.send_event(t_arrival,t_link.dest,'getstate',state_id)
				
	def send_state(self, to, state_id):
		for t_link in self.links:
			if t_link.dest == to:
				#t_block = self.blocks[t_hash]
				#k = random.randint(1,len(self.links))
				t_arrival = t_link.occupy(self.t, 150*len(self.state))
				#print(self.state, t_link.dest,to)
				self.send_event(t_arrival, t_link.dest, 'state', self.state)
	
	def request_tailchain(self, to,state):
		for t_link in self.links:
			if t_link.dest == to:
				t_arrival = t_link.occupy(self.t,0)
				self.send_event(t_arrival,t_link.dest,'tailchain',len(state)-1)
				
	def send_tailchain(self,to,current_height):
		for t_link in self.links:
			if t_link.dest == to:
				for h in self.longest_chain:
					print(h)
					size = self.longest_chain[h].size
					print(size)
					t_arrival = t_link.occupy(self.t,size)
					print(self.t, t_arrival)
					self.send_event(t_arrival,t_link.dest,'addblock',self.longest_chain[h])
	...
	def longest_chain_selection(self):
		...
		while t_hash != self.pulse_block_hash:
			...
			self.S_d[self.blocks[t_hash].height] = n_inputs
			self.S_a[self.blocks[t_hash].height] = n_outputs
			self.state[self.blocks[t_hash].height] =  self.state[self.blocks[t_hash].height-1] - n_tx * n_inputs + n_tx * n_outputs
			#t_size = n_tx * (n_inputs + n_outputs) * 150 + 2048
			#t_block.size = t_size
			if (t_hash not in self.longest_chain):
				#print(self.blocks[t_hash].height)
				#print(len(self.longest_chain))
				...
				self.bitcoin[t_block.height] = t_block
			t_hash = t_block.prev
	...
	def calculate_num_blocks(self):
		x = len(self.longest_chain)
		y = len(self.coin_prune)
		z = len(self.bitcoin)
