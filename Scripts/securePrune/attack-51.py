...
for i in range(numminers):
	miners.append(Miner(i, hashrates[i] * 1.0*9/20.0, 0.5*1024*1024, blocksize,seed_block, event_q, t))

# make the strong miner a bad miner
miners[i] = BadMiner(i, hashrates[i] * 1.0*9/20.0, 0.5*1024*1024,blocksize, seed_block, event_q, t)
...
pylab.grid()
pylab.ylim([-0.1,1.1])
pylab.savefig('rewards_attacker_10.eps')
pylab.draw()
