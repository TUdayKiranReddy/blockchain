...
# batchAdd
# print("No of threads using", (THREAD_MAX))
# x = create_list(2000)
# y = []
# #print(x)
# time_to_batchAdd = []
# for i in range(10):
	# n, A0, S = setup()
	# tik = time.time()
	# A,nipoe = batch_add(A0,S,x[0:(i+1)*200],n)
	# #W = create_all_membership_witnesses(A0, S, n)
	# tok = time.time()
	# print("Lenth:", len(x[0:(i+1)*200]), "\t\tTime Taken: ", tok-tik)
...
# merkle_add_timing = []
# print("Time to calculate MMR from peaks")
# for i in range(5):
	...
	# for j in range(len(x)):
		# merkle_tree.add_leaf(str(x[i]), True)
	# merkle_tree.make_tree()
	# tok = time.time()
	# merkle_add_timing.append(tok - tik)
	# print("Leaves:", len(x), "\t\tTime Taken: ", tok-tik)
# print(merkle_add_timing)

# merkle_tree = merkletools.MerkleTools()

# x = create_list(1000000)
...
# tik = time.time()
# for 
# merkle_tree.validate_proof(proof, merkle_tree.get_leaf(1), merkle_tree.get_merkle_root())
# tok = time.time()
